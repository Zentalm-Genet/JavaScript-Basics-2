...
const divideNumbers = numbers.map(number => number / 2);
console.log(divideNumbers); // Output: [20, 34, 7, 45, 24.5]
...
let cityEndWithI = cities.filter(city => city.endsWith('i'));

console.log(cityEndWithI);
...
// Initialize total price of foods with "rice" tag
let totalPriceRiceTag = 0;

// Iterate over each food item
foods.forEach(meal => {
    // Check if the food has the "rice" tag
    if (meal.tags.includes("rice")) {
        // If yes, add the price to the total price
        totalPriceRiceTag += meal.price;
    }
});

console.log("Total price of foods with 'rice' tag:", totalPriceRiceTag);

// Total price of foods with 'rice' tag: 36
