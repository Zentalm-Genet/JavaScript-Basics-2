...
const findGrapeExpression = function (arr){
    const fruit = arr.find(element => element.color === "purple");
    if (fruit) {
        console.log(`The fruit with index ${arr.indexOf(fruit)} is a grape`);
    }
}
...
const parameter = (p1, p2) => console.log(`The first parameter is ${p1}. The second parameter is ${p2}`)
...
greeting("Zentalm", "Genet", printMyName)
...
function pricesPlusTax(arr, callback){
     ...
     // Use the map method to iterate over the prices array and calculate total cost with tax
    
         newTotal = arr.map(price => price * 1.20) // Add 20% tax
...
// CODE HERE
pricesPlusTax(prices, newTotalCost)
