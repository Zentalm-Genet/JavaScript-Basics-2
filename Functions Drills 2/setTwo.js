...
const last = (arr, callback) => callback(arr.length - 1);
...
// Invoke the each function with the names array and the callback function
...
const addNUMBER = addingFactory(21);
...
function getUserById(users, id, callback) {
  ...
  for (let user of users) {
      ...
      if (user.id === id) {
          // Invoke the callback function with the user object
          callback(user);
          return; // Exit the loop after finding the user
      }
  }
  // If no user with the given id is found, invoke the callback with null
  callback(null);
}
